...
request.setAttribute("yesNoMap",YesNo.yesNoMap);
	...
	*/
	    //保存多对多关联关系
	//保持一对多关联关系
		category.childrenListToSet(request);
	...
	public void updateUI(@ModelAttribute(value="category") Category category,HttpServletRequest request) {
    	...
    	List<Category> topCategories = categoryService.getByProperties(Category.class, new String[] { "status","parent" }, new Object[] { YesNo.Yes,null}, "orderNum", null);
		List<Category> categoryList = new ArrayList<Category>();
		categoryService.tree(topCategories,"",categoryList,category.getId());
		request.setAttribute("categoryList", categoryList);
	 ...
	 * @return Select2<Category> 类目Select2对象
	 */
